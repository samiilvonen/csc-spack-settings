...
class Libpciaccess(AutotoolsPackage):
    ...
    # inw, outw, etc. are macros that are not expanded without optimization
    def setup_environment(self, spack_env, run_env):
        spack_env.append_flags('CFLAGS', '-O')
        spack_env.append_flags('CPPFLAGS', '-O')
        spack_env.append_flags('LDFLAGS', '-O')
